...
const Login = () => {
    ...
    useEffect(() => {
        ...
        if (showPasswordCheckboxRef1.current && passwordInputRef1.current) {
            showPasswordCheckboxRef1.current.addEventListener("change", () => togglePasswordVisibility(passwordInputRef1, showPasswordCheckboxRef1));
        }
        if (showPasswordCheckboxRef2.current && passwordInputRef2.current) {
            showPasswordCheckboxRef2.current.addEventListener("change", () => togglePasswordVisibility(passwordInputRef2, showPasswordCheckboxRef2));
        }
        if (botonDocenteForm1Ref.current) {
            botonDocenteForm1Ref.current.addEventListener("click", () => switchFormVisibility(formDocenteRef, formEstudianteRef));
        }
        if (botonEstudianteForm2Ref.current) {
            botonEstudianteForm2Ref.current.addEventListener("click", () => switchFormVisibility(formEstudianteRef, formDocenteRef));
        }
    
        // Limpiar listeners al desmontar el componente
        return () => {
            if (showPasswordCheckboxRef1.current && passwordInputRef1.current) {
                showPasswordCheckboxRef1.current.removeEventListener("change", () => togglePasswordVisibility(passwordInputRef1, showPasswordCheckboxRef1));
            }
            if (showPasswordCheckboxRef2.current && passwordInputRef2.current) {
                showPasswordCheckboxRef2.current.removeEventListener("change", () => togglePasswordVisibility(passwordInputRef2, showPasswordCheckboxRef2));
            }
            if (botonDocenteForm1Ref.current) {
                botonDocenteForm1Ref.current.removeEventListener("click", () => switchFormVisibility(formDocenteRef, formEstudianteRef));
            }
            if (botonEstudianteForm2Ref.current) {
                botonEstudianteForm2Ref.current.removeEventListener("click", () => switchFormVisibility(formEstudianteRef, formDocenteRef));
            }
        };
    }, [getToken, navigate]);
